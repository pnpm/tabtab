...
const writeToShellConfig = async ({ location, name, shell }) => {
  const scriptname = path.join(
    COMPLETION_DIR,
    `${TABTAB_SCRIPT_NAME}_${shell}.${shellExtension()}`
  );
...
const writeToTabtabScript = async ({ name, shell }) => {
  ...
  const scriptname = path.join(COMPLETION_DIR, `${name}_${shell}.${shellExtension()}`);
...
const writeToCompletionScript = ({ name, completer, shell }) => {
  const filename = untildify(
    path.join(COMPLETION_DIR, `${name}_${shell}.${shellExtension()}`)
  );
...
const uninstall = async (options = { name: '' }) => {
  ...
  const tabtabScript = untildify(
    path.join(COMPLETION_DIR, `${TABTAB_SCRIPT_NAME}_${shell}.${shellExtension()}`)
  );

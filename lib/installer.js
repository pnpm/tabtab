...
const writeToTabtabScript = async ({ name, shell }) => {
  ...
  const scriptname = path.join(COMPLETION_DIR, shell, `${name}.${shellExtension()}`);
...
const writeToCompletionScript = ({ name, completer, shell }) => {
  const filename = untildify(
    path.join(COMPLETION_DIR, shell, `${name}.${shellExtension()}`)
  );
...
const uninstall = async (options = { name: '', shell }) => {
  ...
  const completionScript = untildify(
    path.join(COMPLETION_DIR, options.shell, `${name}.${shellExtension()}`)
  );
  ...
  const tabtabScript = untildify(
    path.join(COMPLETION_DIR, options.shell, `${TABTAB_SCRIPT_NAME}.${shellExtension()}`)
  );
